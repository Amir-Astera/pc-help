 ...
 */
$title = $values = $barsstyle = $units = $bgcolor = $css = $custombgcolor = $customtxtcolor = $options = $el_class = '';
$output = '';
...
$output = '<div class="progress-bars ' . esc_attr( $css_class ) . '">';
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<span class="bar-title"' . $line['txtcolor'] . '>'.$line['label'].'</span>';
    
    $output .= '<div class="vc_single_bar progress shape' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
	...
	$output .= '<div class="vc_bar progress-bar shape ' . esc_attr( implode( ' ', $bar_options ) ) . '" data-percentage-value="' . esc_attr( $percentage_value ) . '" data-value="' . esc_attr( $line['value'] ) . '"' . $line['bgcolor'] . '>';
	$output .= $unit;
    $output .= '</div></div>';
}
