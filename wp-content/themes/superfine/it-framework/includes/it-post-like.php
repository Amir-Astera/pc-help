...
function getPostLikeLink( $post_id ) {
	...
	$count = ( empty( $like_count ) || $like_count == "0" ) ? esc_html__('Like','superfine') : esc_attr( $like_count );
	if ( AlreadyLiked( $post_id ) ) {
		$class = esc_attr( ' liked' );
		$title = esc_attr( esc_html__('Unlike','superfine') );
		$heart = '<i id="icon-like" class="fa fa-heart-o"></i>';
	} else {
		$class = esc_attr( '' );
		$title = esc_attr( esc_html__('Like','superfine') );
		$heart = '<i id="icon-unlike" class="fa fa-heart"></i>';
	}
	$output = '<span class="hidden lk">'.__('Like','superfine').'</span><span class="hidden unlk">'.__('Unlike','superfine').'</span><a href="#" class="jm-post-like f-left shape'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">'.$heart.'&nbsp;'.$count.'</a>';
	return $output;
}
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e( 'You Like:','superfine' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				foreach ( $the_likes as $the_like ) {
					$i++;
					$like_list .= "<a class='f-left shape' href=\"" . esc_url( get_permalink( $the_like ) ) . "\" title=\"" . esc_attr( get_the_title( $the_like ) ) . "\">" . get_the_title( $the_like ) . "</a>\n";
					if ($count != $i) $like_list .= " &middot; ";
					else $like_list .= "</p>\n";
				}
				echo $like_list;
			} else {
				echo "<p>" . _e( 'You don\'t like anything yet.','superfine' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . esc_html__( 'You Like:','superfine' ) . "</h3>\n";
			$like_list .= "<ul>\n";
			foreach ( $limited_likes as $the_like ) {
				$like_list .= "<li><a class='f-left shape' href='" . esc_url( get_permalink( $the_like ) ) . "' title='" . esc_attr( get_the_title( $the_like ) ) . "'>" . get_the_title( $the_like ) . "</a></li>\n";
			}
			$like_list .= "</ul>\n";
			$like_list .= "</aside>\n";
		}
...
function jm_most_popular_today() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'Today\'s Most Popular Posts','superfine' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a class='f-left shape' href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
		}
		echo "</ul>\n";
		echo "</aside>\n";
	}
...
function jm_most_popular_month() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'This Month\'s Most Popular Posts','superfine' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a class='f-left shape' href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular_week() {
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . _e( 'This Week\'s Most Popular Posts','superfine' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a class='f-left shape' href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . _e( 'Most Popular Posts','superfine' ) . "</h3>\n";
	echo "<ul>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_query' => array(
