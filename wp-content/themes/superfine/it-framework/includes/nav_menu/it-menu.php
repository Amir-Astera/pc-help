...
include_once( get_template_directory().'/it-framework/includes/nav_menu/it-edit-form.php' );
include_once( get_template_directory().'/it-framework/includes/nav_menu/it-front-end.php' );
...
function it_nav_menu( $args = array() ) {
    ...
    $effect = theme_option("menu_effect");
    $menu_color = theme_option("sub_menu_color");
    $opts_menu_color = get_post_meta( c_page_ID() , 'sub_menu_color' , true);
    ...
    if ($effect != ''){
        $nav_menu .= '<nav class="top-nav nav-animate '.$effect.' '.$men_col.'">';   
    }else{
